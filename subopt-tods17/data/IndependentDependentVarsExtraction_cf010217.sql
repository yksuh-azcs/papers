-- File created from exp IndependentDependentVarsExtraction_exp102516.sql and irrelevant lines deleted
-- Create table where we calculate suboptimality as no of Std Deviations away
-- for adjacent pairs

-- Source table: NSOCnfm_S4_CTQatC
...
-- If get error that the NOT NULL constraint is violated, run the following statements:
alter table Cnfm_Analysis_SuboptDefn modify (lowcard NUMBER(10) NULL);
alter table Cnfm_Analysis_SuboptDefn modify (pllowcard NUMBER(20) NULL);
 

-- Add rows when there is only one plan for the query with 0 subopt
INSERT INTO Cnfm_Analysis_SuboptDefn 
(SELECT qc1.experimentid, qc1.experimentname, qc1.dbms, qc1.runid, qc1.queryNum, 
  qc1.card as HighCard, qc1.med_calc_qt AS MedianHighCard, 
  ...
  0 AS Subopt_sd, 0 AS subopt_rel 
FROM NSOCnfm_S4_CTQatC qc1
WHERE 1=
(SELECT COUNT(distinct planid)
FROM NSOCnfm_S4_CTQatC qc2
WHERE qc1.experimentid=qc1.experimentid
AND qc1.runid=qc2.runid
...
-- list of Q@Cs
select a1.experimentid, a1.experimentname, a1.dbms, a1.runid, a1.querynum, a1.card FROM
NSOCnfm_S4_CTQatC a1, 
(select experimentid, experimentname, dbms, runid, querynum
from NSOCnfm_S4_CTQatC
group by experimentid, experimentname, dbms, runid, querynum
MINUS
select experimentid, experimentname, dbms, runid, querynum
FROM Cnfm_Analysis_SuboptDefn) a2
WHERE (a1.experimentid=a2.experimentid AND a1.experimentname=a2.experimentname
AND a1.dbms = a2.dbms AND a1.runid = a2.runid AND a1.querynum=a2.querynum)
;
...
tempSJn.NUMSELFJOIN, tempDbOp.CtDbOp, so2.SUBOPT, so2.runid , NVL(NumDiscontOperators,0) AS NumDiscontOperators, tempAgg.NumAgg, tempPK.PK_NoGroupBy, tempPK.PK_TwoSided, tempPK.PK_InGroupBy,
tempPK.PK_InOnesideJoin, tempPK.PK_NoJoin, tempPK.PK_NotInGroupBy, tempPK.PK_NotInJoin, newV0816.pk_pr, newV0816.num_repeats, newV0816.sec_idx_pr, newV0816.sq_pr, newV0816.skew_pr
FROM (
    select experimentid, EXPERIMENTNAME, DBMS, RUNID, QUERYNUM, SUM(subopt_sd) AS subopt from EX_Analysis_SuboptDefn
    GROUP BY experimentid , EXPERIMENTNAME, DBMS, RUNID, QUERYNUM
) so2,
    (SELECT ex.EXPERIMENTID, ex.experimentname, er.starttime, qr.queryId, qr.querynumber, qr.querySQL, qr.RunId, er.dbmsname as dbms FROM AZDBLAB_QUERY qr, AZDBLAB_EXPERIMENTRUN er,
...
INNER JOIN
    (SELECT DISTINCT asd.experimentid, asd.querynum, ps.Value AS PK_NoGroupBy , ps1.Value AS PK_TwoSided, ps2.Value AS PK_InGroupBy, ps3.Value AS PK_InOnesideJoin, ps4.Value AS PK_NoJoin, 
      ps5.Value AS PK_NotInGroupBy, ps6.Value AS PK_NotInJoin
    FROM Cnfm_Analysis_SuboptDefn asd LEFT OUTER JOIN azdblab_pkqueryparam ps ON (ps.PARAMNAME = 'NoGroupBy' AND asd.queryNum = ps.queryNumber AND asd.experimentid = ps.experimentid) LEFT OUTER 
      JOIN azdblab_pkqueryparam ps1 ON (ps1.PARAMNAME = 'PK_InBothSideJoin' AND asd.queryNum = ps1.queryNumber AND asd.experimentid = ps1.experimentid) LEFT OUTER JOIN azdblab_pkqueryparam ps2  
      ON (ps2.PARAMNAME = 'PK_InGroupBy' AND asd.queryNum = ps2.queryNumber AND asd.experimentid = ps2.experimentid ) LEFT OUTER JOIN azdblab_pkqueryparam ps3 ON (ps3.PARAMNAME = 
      'PK_InOnesideJoin' AND asd.queryNum = ps3.queryNumber AND asd.experimentid = ps3.experimentid) LEFT OUTER JOIN azdblab_pkqueryparam ps4 ON (ps4.PARAMNAME = 'PK_NoJoin' AND asd.queryNum = 
      ps4.queryNumber AND asd.experimentid = ps4.experimentid) LEFT OUTER JOIN azdblab_pkqueryparam ps5 ON (ps5.PARAMNAME = 'PK_NotInGroupBy' AND asd.queryNum = ps5.queryNumber AND 
      asd.experimentid = ps5.experimentid) LEFT OUTER JOIN azdblab_pkqueryparam  ps6  ON (ps6.PARAMNAME = 'PK_NotInJoin' AND asd.queryNum = ps6.queryNumber AND asd.experimentid = ps6.experimentid)
    ) tempPK
        ON temp.queryNumber=tempPK.queryNum ANd temp.experimentId=tempPk.experimentId
INNER JOIN NSOCnfm_S4_New_Var newV0816 ON  newV0816.queryNum=temp.queryNumber AND newV0816.dbms=temp.dbms AND newV0816.runid=temp.runid
LEFT OUTER JOIN
...
GROUP BY queryid
    ...
    INNER JOIN (SELECT er.dbmsname AS DBMS, qr.QueryID AS QID, count(DISTINCT pl.PlanID) AS CEPS FROM AZDBLAB_PLAN pl,
    ...
    GROUP BY (ex.ExperimentName, er.DBMSname, qr.QueryID)) tempCeps ON temp.queryId=tempCeps.QID
    INNER JOIN
    ...
    INNER JOIN
    (SELECT er.DBMSname AS DBMS, COUNT(DISTINCT po.OperatorName) AS CtDbOp FROM AZDBLAB_EXPERIMENT ex,
    ...
    AND qn.QueryExecutionID = hp.QueryExecutionID AND hp.PlanID = po.PlanID GROUP BY er.DBMSname) tempDbOp
...
select lowcard, count(queryNum), min(Subopt_rel), max(Subopt_rel), min(MedianHighCard), max(MedianHighCard) from EX_Analysis_SuboptDefn_V1113 group by lowcard 
order by lowcard


 select experimentid, dbms, runid, queryNum, max(Subopt_rel) as max_sor from EX_Analysis_SuboptDefn_V1113 group by experimentid, dbms, runid, queryNum 
having max(Subopt_rel)>0

 select experimentid, dbms, runid, queryNum, max(Subopt_rel), min(Subopt_rel) from EX_Analysis_SuboptDefn_V1113 group by experimentid, dbms, runid, queryNum 
...
CREATE TABLE NumOperatorsPerCardinality AS
SELECT er.experimentid, er.dbmsName AS dbms, er.runid, qu.querynumber, qe.queryid, Cardinality, qep.PLANID, COUNT(DISTINCT po.operatorname) AS
...
ON (qu.runid=er.runid) INNER JOIN CF_Analysis_S4_CTQatC_Ver1 qc1 ON (qc1.experimentid =er.experimentid AND qc1.DBMS=er.dbmsname AND qc1.runid=er.runid AND qc1.querynum=qu.querynumber 
AND qc1.planid=qep.planid AND qc1.card=qe.cardinality AND qc1.planid=qep.planid)
GROUP BY er.experimentid, er.dbmsname, er.runid, qu.querynumber, qe.queryid, qep.PLANID, cardinality, qc1.med_calc_qt;
