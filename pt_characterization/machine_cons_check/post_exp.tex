...
Similarly, we looked at the data from INC16 on {\tt sodb12}. 
...
\end{center}
\vspace{-.2in}
\caption{Detailed description of INC128 data\label{tab:exp_notes5}}
\end{table}


Table~\ref{tab:daemon2} shows a list of daemon processes observed at some iterations for which more than three daemons revealed positive PT while the INC16 program was run.
\begin{table}[htp!]
\centering
{
 \begin{tabular}{|p{1.5cm}|p{2cm}|p{12.5cm}|} \hline
Machine Name & Iteration \# & Process Name (id, PT(msec))\\ \hline
 {\tt sodb12}  & 33 & {\tt flush-9:0} ({\tt 25187}, 1), {\tt java} ({\tt 32627}, 3), {\tt java} ({\tt 304}, 1), {\tt md0\_raid1} ({\tt 484}, 1) \\ \cline{2-3}
 ...
 \caption{Some daemon processes captured across different machines~\label{tab:daemon2}}
\end{table}