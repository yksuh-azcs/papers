...
ymax <- ceiling(ymax/100)*100
#ymax <- 120
plot(h, freq=TRUE,ylim=c(0,ymax), xlim=c(xmin-6,xmax+12),col="blue", main='ET frequency on INC32', 
sub=paste("(n=",nrow(x),", bin_size=",binsize,"ms)",sep=""), 
xlab='ET (ms)', ylab=expression('Frequency'))
#xaxtl <- seq(xmin-1,xmax+1,by=1)
#axis(side=1, at=xaxtl, labels=xaxtl)
...
ymax <- ceiling(ymax/100)*100
ymax <- 100
plot(h, freq=TRUE,ylim=c(0,ymax), xlim=c(xmin-18,xmax+14),col="blue", main='ET frequency on INC128', 
sub=paste("(n=",nrow(x),", bin_size=",binsize,"ms)",sep=""), 
xlab='ET (ms)', ylab=expression('Frequency'))
dev.off()
