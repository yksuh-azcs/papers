...
%
%\item We propose a more accurate and precise protocol based on the identification variance 
%variance for that program.

\item We present an algorithm to identify and eliminate such daemons that are infrequent, long-running.
...
{\color{blue}
\tcc{{\color{blue}{\scriptsize Daemon-cutoff times are derived from Steps~2 through~9.}}}
...
({\color{blue}A different Linux distribution may yield different cutoff data than Tab.~\ref{tab:final_infrequent_cutoff}.})
}
...
we discarded any \hbox{sample} \hbox{containing} an infrequent, long-running daemon execution
over the corresponding cutoff.} We thus ended up dropping just fifteen of the 800
